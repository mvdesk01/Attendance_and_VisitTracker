...
class _LeaveDetailsPageState extends State<LeaveDetailsPage> {
  ...
  //
  //
  //     if (widget.leaveData.fromTime != null && widget.leaveData.fromTime!.isNotEmpty) {
  //       String formattedFromTime = _formatTimeTo12Hour(widget.leaveData.fromTime!);
  //       fromTimeController.text = formattedFromTime;
  //     }
  //
  //     // ✅ FIX: Format toTime to only show time (hh:mm a)
  //     if (widget.leaveData.toTime != null && widget.leaveData.toTime!.isNotEmpty) {
  //       String formattedToTime = _formatTimeTo12Hour(widget.leaveData.toTime!);
  //       toTimeController.text = formattedToTime;
  //     }
  //
  //     // Set leaveInFraction based on whether fromTime/toTime have values
  //     if (widget.leaveData.fromTime != null && widget.leaveData.fromTime!.isNotEmpty &&
  //         widget.leaveData.toTime != null && widget.leaveData.toTime!.isNotEmpty) {
  ...
  //     // }
  //
  //     if(fromTimeController.text != null && fromTimeController.text.isNotEmpty){
  //       _calculateTotalTime();
  //     }
  ...
  void initState() {
    ...
    if (widget.flag == 2) {
      ...
      // ✅ MORE ROBUST: Check if time values are valid (not empty, not default, and within working hours)
      bool hasValidFromTime = _isValidTimeValue(widget.leaveData.fromTime);
      bool hasValidToTime = _isValidTimeValue(widget.leaveData.toTime);

      // Check if total days has a fractional value (indicates fraction leave)
      bool hasFractionalDays = false;
      if (widget.leaveData.noOfDays != null) {
        double days = double.tryParse(widget.leaveData.noOfDays.toString()) ?? 0;
        hasFractionalDays = (days - days.floor()) > 0.01; // Has decimal part
      }

      // Leave in fraction is true ONLY if we have valid time values AND fractional days
      if (hasValidFromTime && hasValidToTime && hasFractionalDays) {
        leaveInFraction = true;

        // Format and set the time values
        String formattedFromTime = _formatTimeTo12Hour(widget.leaveData.fromTime!);
        fromTimeController.text = formattedFromTime;
        ...
        if (fromTimeController.text.isNotEmpty && toTimeController.text.isNotEmpty) {
          _calculateTotalTime();
        }
      } else {
...
// Add this helper method to check if time value is valid
  ...
  String _formatTimeTo12Hour(String timeString) {
    try {
      ...
      if (timeString.contains(':') && (timeString.length == 5 || timeString.length == 8)) {
        // Check if it's 24-hour format (like "14:30")
  ...
  Widget build(BuildContext context) {
    return BlocListener<MainBloc, MainState>(
        ...
        if(state is GetSubmitLeaveLoadingState){
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (Navigator.canPop(context)) {
              Navigator.pop(context, true); // pass result
            }
          });
        }
      ...
      child: Scaffold(
        appBar: AppBar(title: Text(widget.flag == 2 ? 'Update Details' : 'Enter Details')),
                    ...
                    child: ElevatedButton(
                      ...
                      child: isLoading
                        ...
                        child: CircularProgressIndicator(
                          ...
                          : Text(
                        widget.flag == 2 ? 'Update Details' : 'Submit Leave',
...
// To Date and Time
                ...
                ElevatedButton(
                  ...
                  child: isLoading
                      ? CircularProgressIndicator() // Show loader when submitting
                      : Text(widget.flag == 2 ? 'Update Details' : 'Submit Leave'),
                ),
              ],
            ),
          ),
  ...
  _addLeavedata() {
    ...
    SubmitLeaveDetails leaveDetails = SubmitLeaveDetails(
      flag: widget.flag == 1 ? "INSERT" : "UPDATE",
      transactionId: widget.flag == 1 ? "" : "55063",
      staffCode: staff_code,
      name: staff_name,
      plant: plant_name,
      doj: joining_date,
